      ...
      <View style={tw`flex-grow flex flex-col items-center pt-16 pb-24 px-4`}>
        <View style={tw`w-full max-w-md flex flex-col items-center`}>
          ...
          <View style={tw`relative w-[140px] h-[140px] mx-auto mb-5 mt-2`}>
            {/* White Card */}
            <View style={tw`absolute inset-0 bg-white rounded-[32px] shadow-[0_8px_40px_rgba(0,0,0,0.04)] flex flex-col items-center justify-center overflow-hidden`}>
              {/* Top right circle decoration */}
              <View style={tw`absolute -top-4 -right-4 w-16 h-16 bg-[#F8FAFC] rounded-full`}></View>

              {/* Light Blue Inner Square */}
              <View style={tw`relative w-[60px] h-[60px] bg-[#E0EAFF] rounded-[20px] flex items-center justify-center mb-4 z-10`}>
                <Text style={tw`hidden text-[#0057FF] text-[32px]`}>group</Text>
              </View>
              {/* Decorative Pills */}
              <View style={tw`relative flex gap-2 z-10`}>
          ...
          <View style={tw`text-center mb-5 px-2`}>
            <Text style={tw`text-[18px] font-bold text-slate-900 mb-2 leading-tight`}>No Active
              Queues</Text>
            <Text style={tw`text-slate-600 text-[13px] leading-relaxed`}>You are not waiting in any virtual queue at the moment. Need to see a doctor?</Text>
          </View>
          ...
          <View style={tw`w-full space-y-2.5 mb-6`}>
            ...
            ><Text>Join Live Queue</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => navigation.navigate('Departments')}
            ...
            ><Text>View Specialties</Text></TouchableOpacity>
          </View>
